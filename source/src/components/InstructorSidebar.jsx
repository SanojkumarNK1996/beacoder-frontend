...
// Sidebar options for instructors - only 3 options
const instructorSidebarOptions = [
    { label: "Assignments", icon: <FaTasks />, path: "/instructor/assignments" },
    { label: "Discussions", icon: <FaComments />, path: "/instructor/discussions" },
    { label: "Pair Programming", icon: <FaUsers />, path: "/instructor/pair-programming" },
];


export const InstructorSidebar = ({ hovered, setHovered }) => {
    const location = useLocation();
    // Find the index of the sidebar option that matches the current route
    const activeIdx = instructorSidebarOptions.findIndex(option => location.pathname.startsWith(option.path));
        ...
        >
            <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: "45px" }}>
                ...
                <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "16px" }}>
                    {instructorSidebarOptions.map((option, idx) => (
                        <Link
                            to={option.path}
                            key={option.label}
